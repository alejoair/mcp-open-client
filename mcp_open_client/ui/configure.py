...
def show_content(container):
    ...
    with container:
        ...
        config = app.storage.user.get('user-settings', {})
        ...
        # Add info about refreshing models when changing API settings
        ui.label('💡 Tip: Click "Refresh Models" after changing API settings to load available models').classes('text-caption text-blue-600 mt-2')
        ...
        async def load_models():
            ...
            current_model = config.get('model', 'claude-3-5-sonnet')
            ...
            with model_select_container:
                ...
                original_config = app.storage.user.get('user-settings', {})
                ...
                with ui.row().classes('w-full items-center mt-2'):
                    ui.button('🔄 Refresh Models', on_click=load_models).props('size=sm color=secondary')
                    ui.label('Reload models using current API settings').classes('text-caption text-gray-600')
        ...
        def create_initial_model_select():
            ...
            with model_select_container:
                ...
                with ui.row().classes('w-full items-center mt-2'):
                    ui.button('🔄 Load Models from API', on_click=load_models).props('size=sm color=primary')
                    ui.label('Click to load available models from your API').classes('text-caption text-gray-600')
                
                ui.label('ℹ️ Using default models. Click "Load Models from API" to get models from your server.').classes('text-caption text-blue-600 mt-1')
        ...
        def save_config():
            ...
            update_current_config_display()

        ui.button('Save Configuration', on_click=save_config).props('color=primary')

        # Display current configuration
        current_config_expansion = ui.expansion('Current Configuration', icon='settings').classes('w-full')
        
        def update_current_config_display():
            current_config = app.storage.user.get('user-settings', {})
            with current_config_expansion:
                current_config_expansion.clear()
                ui.label('API Key: ' + ('*' * len(current_config.get('api_key', ''))))
                ui.label('Base URL: ' + current_config.get('base_url', 'Not set'))
                ui.label('Model: ' + current_config.get('model', 'Not set'))
        ...
        def confirm_reset():
            with ui.dialog() as dialog, ui.card():
                ui.label('Reset to Factory Settings').classes('text-h6')
                ui.label('This will overwrite your current configuration with the factory defaults.')
                with ui.row():
        ...
        ui.button('Reset to Factory Settings', on_click=confirm_reset).props('color=secondary')
